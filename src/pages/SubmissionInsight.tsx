      ...
      try {
        ...
        if (response.code === 200 && response.data) {
          ...
          if (response.data.submissions) {
            const formattedSubmissions = response.data.submissions.flatMap(
              (userSubmission: any) => {
                return userSubmission.submissions.map(
          ...
          setStats({
            ...
            conversionRate:
              uniqueUsers > 0
                ? Math.round((totalResponses / uniqueUsers) * 100)
                : 0,
          });
        } else {
      ...
      <div className="p-6">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-[var(--color-black-900)] mb-2">
              Error Loading Data
            </h2>
      ...
      <div className="mb-8">
        ...
        {formId && (
          <div className="mt-2 p-3 bg-[var(--color-light-bg)] rounded-lg border border-[var(--color-light-border)]">
            ...
            <p className="text-sm text-[var(--color-black-600)]">
              <strong>API Endpoint:</strong> /api/form-submissions/detail/
              {formId}
            </p>
            {formData && (
              <div className="mt-2 pt-2 border-t border-[var(--color-light-border)]">
                ...
                <p className="text-sm text-[var(--color-black-600)]">
                  ...
                  {formData.summary?.total_submissions || 0}
                </p>
                <p className="text-sm text-[var(--color-black-600)]">
                  <strong>Unique Users:</strong>{" "}
                  {formData.summary?.unique_users || 0}
                </p>
              </div>
            )}
          </div>
